...
- 文を直したら、まず `kana` で読みを確認する。読み違いは:
  - 語の読み・アクセント → `dictionary.json` に登録（例: 勤務時間＝キンムジカン、月末＝ゲツマツ、右上＝ミギウエ）
  - 無声化した「ヒ」が「人（ひと）」に聞こえる → 自動で有声に直している（`lib/narrate.mjs`）
  - 数字との組み合わせ（例:「10月末」→ じゅうげつまつ）や送り仮名（「開けます」→ あけます）→ 原稿の書き方を変える（「10月の月末」「開きます」）
...
オーナーから受け取るスクリーンショット（iPhone の「照会カレンダーを追加」など）は、**そのまま貼らずに模型で再現**する。
写真には本人専用の登録用URL（知っている人は誰でもその人のシフトを見られる）などが写っていることがあるため。
模型では URL・ID を架空のもの（`a1b2c3…`）にする。
