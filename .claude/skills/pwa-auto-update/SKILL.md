...
- `assets/generate-sw.mjs` — build-time generator → `public/sw.js`, stamped with `SW_VERSION` (git short SHA; fallback `Date.now()`). The SW has **no `fetch` listener**; on `activate` it clears stale caches and `clients.claim()`s. It answers two messages: `SKIP_WAITING` and `GET_VERSION` (replies with `SW_VERSION` over the passed `MessagePort`).
