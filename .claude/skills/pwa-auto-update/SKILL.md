...
- `assets/ReloadPrompt.tsx` — `<ReloadPrompt />` banner using plain `navigator.serviceWorker` (no deps). Its update button shows an **"updating…" state**, posts `SKIP_WAITING`, and reloads once the new SW reaches `activated` (or on `controllerchange`), with a fallback timer for iOS. It de-duplicates notifications **two ways**: per waiting SW instance (within a page) and per `SW_VERSION` in `localStorage` (across reloads), so one deploy shows the banner exactly once.
- `assets/generate-sw.mjs` — build-time generator → `public/sw.js`, stamped with `SW_VERSION` = a **content hash of the app files only** (sha256 of `git ls-files -s -- $APP_PATHS`, 12 chars; `-dirty<ts>` suffix when those paths have uncommitted changes; `Date.now()` fallback outside git). Tune `APP_PATHS` per project — anything not in it (docs, README, CI config) can be deployed without prompting users to update. The SW has **no `fetch` listener**; on `activate` it clears stale caches and `clients.claim()`s. It answers two messages: `SKIP_WAITING` and `GET_VERSION` (replies with `SW_VERSION` over the passed `MessagePort`).
...
Also verify the version is **content-addressed, not commit-addressed** (clean tree):
`node scripts/generate-sw.mjs` twice → same version; commit a docs-only change, run again → **still the same version**; touch any file under `APP_PATHS` → version changes. After deploying, `curl -s https://<prod>/sw.js | grep SW_VERSION` should match what the generator prints locally on the same commit.
...
- **iOS standalone: don't depend on `controllerchange` alone to reload.** iOS Safari (home-screen) doesn't reliably fire it, so relying on it makes the button look dead. The banner posts `SKIP_WAITING` best-effort, then reloads — on the new worker reaching `activated` or on `controllerchange`, otherwise a fallback timer forces `window.location.reload()`. Large tap target + `touch-action: manipulation`.
- **🔴 Don't reload on a short fixed timer after `SKIP_WAITING` — that is what causes a *second* banner for one deploy.** With a blind `setTimeout(reload, 800)`, iOS standalone often doesn't finish activating in time, so **the page reloads while the new SW is still `waiting`**. After the reload, the per-instance ref is gone but `reg.waiting` is still there, so the same deploy prompts again. Fix both halves: (1) wait for `statechange → "activated"` (fallback ~2.5s), and (2) persist which version was already handled (below).
- **Give the update button a visible "updating…" state.** A same-tab reload gives no feedback that the tap registered, so users tap repeatedly. On click, switch the label to "更新中…"/"Updating…" and disable the button (hide the ✕) *before* reloading; the wait for activation is what makes that state briefly visible.
...
- **That ref is not enough — also persist the handled version, or the banner returns after any reload.** The ref dies with the page, so a reload (including the update button's own reload, a ✕-dismiss followed by reopening the app, or any full page load) re-shows the *same* deploy's banner. Ask the waiting worker for its version (`GET_VERSION` over a `MessageChannel`; the generated SW replies with `SW_VERSION`) and record it in `localStorage` when the user taps update **or** dismisses with ✕; skip `showBanner` for an already-recorded version. Wrap `localStorage` access in try/catch (private mode) and fall back to the ref-only behavior when the version can't be read.
- **Recording "handled" cannot strand a user on an old build** — and this is exactly why the persistence is safe here. This SW has no `fetch` handler, so **page content always comes from the network**: any reload already yields the newest app. The waiting/active state of the SW is about the SW script alone, not the app version. The banner is only a "there is a new deploy, reload when convenient" hint, so showing it once per deploy loses nothing. (This reasoning does **not** transfer to a caching SW, where the waiting worker really does gate the content.)
...
- **🔴 Don't stamp `SW_VERSION` with the git SHA — docs-only deploys then nag every user to update.** With push-to-deploy (Cloudflare Workers Builds, Vercel, Netlify…), a commit touching only `docs/` or `README.md` still rebuilds and redeploys, so a SHA-stamped SW changes and every user gets an "update available" banner for a change that cannot affect them (observed in production: a doc commit shipped `SW_VERSION = "fc6b628"` and prompted the fleet). Stamp a **content hash of `APP_PATHS`** instead, as `assets/generate-sw.mjs` now does, so the version only moves when the app itself moves.
  - Use `git ls-files -s -- <paths>` (index blob hashes, no history). **Do not** derive the version from history (`git log -1 --format=%h -- <paths>`): CI clones are usually shallow (`depth=1`), so for a docs-only commit that command returns empty and you silently fall back to a timestamp — a banner on every deploy, the very bug you were fixing.
  - Keep a `-dirty<timestamp>` suffix for uncommitted trees, or local preview builds stop being detected as updates.
  - The changeover deploy itself still prompts once (the SW file's bytes change); the benefit starts with the next docs-only deploy.
  - Complementary, host-side: most push-to-deploy hosts can skip the build entirely for such commits (Cloudflare: *Worker → Settings → Build → Build watch paths*, default include `*` / exclude none — add `docs/*` and `*.md` to **Exclude paths**). That also saves build minutes, but it lives in the dashboard, not the repo, so it is lost when the project is recreated elsewhere; the content-hash version travels with the code. Doing both is fine.
