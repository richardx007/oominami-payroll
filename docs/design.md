...
app/
  (employee)/            従業員向け（スマホ基本+PC/iPadは2カラム、下部タブナビ）
    layout.tsx           スマホ:max-w-lg / lg以上:max-w-5xl
...
- 用途: ①招待メール ②給与明細配信 ③連絡・催促（メール併送チェック）④税理士へ支給一覧（CSV添付）。
- `Message-ID` ヘッダー付き（迷惑メール判定対策）。
- **添付対応**: `smtpSendMail` は `attachments` を受け取り multipart/mixed で送信。
  税理士メールは BOM 付き CSV（`payroll_YYYY-MM.csv`）を添付する。
