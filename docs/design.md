...
- `computeIncomeTax()`: 源泉所得税。
  ...
  - 🆕 **Excelファイルからの自動入力（2026-08-22追加）**: 上記バグの原因調査で「手動コピペでは
    ...
    - 実装したロジックを**オーナー実際の令和8年分ファイルで検証**: 抽出結果232行と、既存DBの
      ...
      本体コードには影響しない検証作業）。
    - `admin/settings/ui.tsx`の`TaxTableForm`にファイル選択入力を追加。選択すると`xlsx`ライブラリを
      **動的import**（この画面でしか使わないため設定ページの初期バンドルに含めない）し、
      パース結果で「対象年」欄・貼付欄（テキストエリア）を**自動入力するだけ**（自動送信はしない。
      税務データのため内容を確認してから「取り込む」を押す運用を維持）。読み取り失敗時は
      エラーメッセージを出し、従来の手動コピペにフォールバックできる。
