...
- 「PDF」を押すといきなりダウンロードするのではなく、**プレビューのダイアログを開き**、そこから
  出力方法を選ぶ。**スマホ（PDFファイルの共有ができる端末）は「共有」だけ**を主ボタンとして出し、
  意味のない「ダウンロード」は出さない。**PC（共有シートが無い）は「ダウンロード」だけ**を出す
  （2026-09-11、オーナー依頼で出し分けに変更。判定は `navigator.canShare({files})`）。実装の考え方は **biz-management の請求書プレビュー**
