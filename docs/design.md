...
- **深夜勤務手当**: 勤務時間のうち **22:00〜翌5:00** に該当する分数（`lib/period.ts` の `nightMinutes()`。出退勤区間と毎日の深夜帯の重なりの合計。退勤が出勤以前なら翌日跨ぎとして計算。休憩がいつ取られたかは記録が無いため休憩分は差し引かない）に対し、**時給の25%**を割増手当として基本給とは別に追加支給する（日ごとに切り捨て）。課税対象額・総支給額に含める。明細（アプリ・メール）と締め処理表・税理士CSVに深夜勤務時間/手当の内訳を表示（深夜勤務があるときのみ）。
...
- 予実一覧のヘッダー右には色凡例の代わりに「予定と実績の不一致は**赤字**」の注釈を表示（"赤字"のみ赤太字＝色見本兼用）。
