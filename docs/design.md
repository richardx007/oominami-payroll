...
app/
  (employee)/            従業員向け（スマホ基本+PC/iPadは2カラム、下部タブナビ）
    ...
    timesheet/           勤務表カレンダー入力（page/ui/actions）
                         ...
                         入力欄は白背景・大きめフォント（高齢者配慮）・出退勤は縦積み（iOS重なり回避）
  ...
  admin/                 管理者向け（レスポンシブ。md以上は左サイドバー/スマホは上部ヘッダー、ネイビー）
    ...
    nav.tsx              Logo / AdminSidebarNav / AdminTopNav（現在ページをハイライト）
                         メニュー順: ダッシュボード / 勤務時間 / 締め処理 / 従業員 / 連絡 / 税理士資料 / 設定
    loading.tsx          画面遷移中のスピナー（連打防止・iPad体感改善）
    page.tsx             入力状況ダッシュボード（年月大表示・状態バッジ・前後月ボタン）
    DashboardCalendar.tsx  給与期間カレンダー。日ごとに勤務者数のドット、日クリックで
                         その日の勤務者一覧（氏名/勤務開始〜終了/勤務時間/交通費）を右に表示
    hours/               勤務時間画面（page/ui）。年月＜＞ナビ、左=当月勤務した従業員一覧、
                         選択で右にその従業員の月次明細（年月日/曜日/開始〜終了/勤務時間・交通手段・
                         区間1・往復⇔片道→・区間2/交通費）。合計は太字
    employees/           従業員管理（登録・氏名/メール編集・時給・税区分・退職・招待・パスワード再設定）
                         区分(管理者M/従業員E)を選んで自動採番、編集は吹き出しパネル
    close/               締め処理（プレビュー・締め・支払済み・明細メール配信）
                         明細配信は送信元アドレス（0円明細）を宛先から除外
    notices/             連絡・催促・一斉報知の送信。個別=管理者にCC / 一斉=管理者にも配信
    report/              税理士向け給与支給一覧（印刷/PDF・CSVダウンロード・税理士へメール自動送信）
    settings/            メール設定（会社名/送信元/税理士 氏名・アドレスを2カラム）・昼食補助・源泉徴収税額表
  login/                 ログイン
  register/              初回登録（メールのみ入力→マジックリンク送信）
  set-password/          マジックリンク/再設定リンク後のパスワード設定
  auth/callback/         Supabase 認証コールバック。token_hash+verifyOtp(再設定) と
                         code+exchangeCodeForSession(初回登録) の両対応。setup=1/recovery で /set-password へ
...
- **管理者によるパスワード再設定（`resetEmployeePassword`）**:
  - 従業員管理画面で「登録済み・在籍中」の従業員に「パスワード再設定」ボタンを表示。
  - サーバー側で `resetPasswordForEmail(email, { redirectTo: /auth/callback?setup=1 })` を送信。
  - **⚠️ PKCE の落とし穴と対策**: 既定の PKCE 方式では照合用 `code_verifier` が「リンクを発行した
    ブラウザ」の Cookie に紐づく。再設定は**管理者のサーバー処理**が発行するため、その値は従業員側に
    存在せず、`exchangeCodeForSession` は必ず失敗する（→ ログイン画面に戻る）。
  - このため再設定リンクは **`token_hash` + `verifyOtp` 方式**にした（verifier 不要でどのブラウザでも検証可）。
    `/auth/callback` は `token_hash` があれば `verifyOtp({token_hash, type})`、なければ従来どおり
    `code` を `exchangeCodeForSession` で処理する（初回登録は本人ブラウザ発行なので従来のまま動く）。
  - **これは Supabase 側の設定変更が1つ必須**: Authentication → Emails →「Reset password」テンプレートの
    リンクを `{{ .SiteURL }}/auth/callback?token_hash={{ .TokenHash }}&type=recovery&setup=1` にする
    （既定の `{{ .ConfirmationURL }}` のままだと PKCE リンクになり再設定が壊れる。「Reset template」で
    初期化しても壊れる）。この認証パターンはスキル `.claude/skills/supabase-invite-auth/` に文書化済み。
  - サービスロールキーは不要（anon/公開キー + ユーザーセッションのみ）。
...
- **「Reset password」テンプレート（パスワード再設定で必須）**: リンクを
  `{{ .SiteURL }}/auth/callback?token_hash={{ .TokenHash }}&type=recovery&setup=1` に設定済み。
  既定の `{{ .ConfirmationURL }}`（PKCE）に戻すと管理者発行の再設定が壊れる（詳細は「4. 認証・ロール」）。
