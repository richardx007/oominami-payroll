...
- 登録: 設定画面の最後「アプリの解説」（`admin/settings/guides.tsx`、アンカー `#app-guides`）。タイトル・URL・概略・公開対象（管理者／従業員のチェック）。
  ...
  一覧の2行目（解説の下）: 動画の項目は「作成日時」（`video_created_at`。マイグレーション `20260926020000_app_guide_video_created_at.sql`）、
  URL の項目は「更新日時」（`updated_at`＝保存した日時）。どちらも**自動**で、手入力はしない（2026-09-26 オーナー依頼）。
  - 動画の作成日時は、動画を選んだときに MP4/MOV の `moov > mvhd` の creation_time を読む（`lib/mp4-meta.ts` の `readMp4CreationTime`。
    ボックスの見出しと mvhd の先頭だけを slice で読むので大きな動画でも軽い。テスト `lib/mp4-meta.test.ts`）。
    iPhone・カメラの動画には入っている。**ffmpeg で書き出した動画は `-metadata creation_time=...` を付けないと 0** で、
    そのときはファイルの更新日時（`File.lastModified`）を使い、画面にその旨を出す。
...
- Google ドライブの動画は共有設定を「リンクを知っている全員」（閲覧者）にしないと、従業員のアカウントでは開けない。

### 26.3 動画をアプリに保存して再生（2026-09-26追加。マイグレーション `20260926010000_app_guide_videos.sql`、本番適用済み）
- 背景: Google ドライブの動画は iPhone（特にホーム画面に追加したアプリ）で小さいプレビューにしかならず、Google にログインしていない状態では
  「何らかの問題により、この動画を再生できませんでした」になることがあった（ドライブ側の制限でアプリからは直せない）。
- 各項目は「🎬 動画をアプリに保存」か「🔗 URL」のどちらか（`url` と `video_path` のどちらかが必須。`app_guides_link_check`）。
- 動画は非公開バケット `app-guides`（1ファイル50MB・mp4/mov）に `ランダムID.mp4` で保存。**ブラウザから Storage へ直接アップロード**し、
  サーバーアクションはパスと大きさだけ記録する（Workers を通すと CPU 時間10ms・サーバーアクションの5MB上限に当たるため）。
  保存に失敗したらアップロード済みの動画を消す（`discardGuideVideo`）。差し替え・URLへの切替・削除のときは前の動画を消す。
- Storage の SELECT も公開対象で絞る（管理者は全部、従業員は `for_employee` の解説の動画だけ）。
- 再生は `/watch/[id]`（メニューの枠なし・黒背景・`<video controls playsInline>` を画面いっぱいに `object-contain`）。
  署名付きURL（4時間）。横向きのスマホ（高さ500px以下の横長）では見出し・概略を隠し、左上に×だけ重ねる。「⛶ 全画面」は iPhone では
  `webkitEnterFullscreen`（標準プレイヤー）。SW は fetch を横取りしないので範囲リクエスト（シーク）もそのまま Supabase に届く。
- 無料枠の目安: 保存容量1GB・通信量 月5GB。設定画面に保存済み動画の合計を表示。軽量化した動画（約2分で10MB台）なら20本でも容量は足り、
  通信量は月300回程度の再生まで。
