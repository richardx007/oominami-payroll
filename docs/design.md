...
- **時刻の丸め**: 設定 `clock_round_min`（分）で丸める。**出勤=切り上げ / 退勤=切り捨て**
  ...
  - 🔴 **出勤の切り上げは24:00をまたぐことがある**（例: 23:50・丸め30分 → 24:00 = 翌日0:00）。
    `roundTime()`（`clock/actions.ts`）はこの場合 `dayOffset=1` を返し、`work_date` も1日進める
...
- 各割当に**変則勤務時間**（`shift_assignments.custom_start`/`custom_end`。例「20:00」）を任意で設定でき、
  ...
  未入力時は入力欄に**枠の既定時刻を薄いグレー文字で表示**（`text-gray-400`。実値は空のまま＝枠既定を使う扱い。
  ユーザーが操作すると濃い文字の明示的な値になる。2026-07-24）。
...
### 11.3.3 🔴 勤務実績と紐付かない前払金（孤立レコード）

前払金は `(employee_id, work_date)` で勤務実績と結びつけているが、**実体は別テーブル**で
外部キー制約は無い。そのため**勤務実績を削除したり、打刻修正で勤務日がずれたりすると
前払金だけが取り残される**。

- 孤立レコードは**日別実績の表に現れない**（表は勤務実績の行を並べたもの）のに、
  **給与計算では差引支給額から控除され続ける**（`payroll-data.ts` は期間内の
  `advance_payments` を日付範囲で合算するだけで、勤務実績の有無を見ない）。
  → 放置すると「理由の分からない控除」になる。
- **対策**: `loadDailyReport` が孤立レコードを検出して `DailyReport.orphanAdvances` で返し、
  日別実績の画面上部に**赤い警告**として出す（管理者のみ。従業員は対処できないため出さない）。
- ⚠️ **勤務実績の日付を変える／削除するときは、前払金も合わせて確認すること。**
  2026-08-01に実際に発生した（打刻修正で `work_date` を 7/31→8/1 に直したが前払金が 7/31 に
  取り残された）。なおこのときは 7/31・8/1 が同一給与期間（2026年8月度＝7/26〜8/25）
  だったため控除額に影響は無かったが、**月をまたぐ修正では控除される期間が変わってしまう**。
