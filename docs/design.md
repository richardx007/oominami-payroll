...
- **時給・税区分の履歴編集UI（`admin/employees/ui.tsx`の`WageHistory`/`TaxHistory`。2026-07-24整理）**:
  ...
  編集フォームも「適用開始日（左）→値（右）」の順で、`min-w-0`を付けて`<input type=date>`が
  枠からはみ出さないようにしている。一覧の下に新規追加フォーム（同じ左右順）があり、追加ボタンは
  右寄せ・「更新」ボタンと同サイズ（`px-4 py-2`）。税区分履歴の訂正・削除は`editTaxSetting`/
  `deleteTaxSetting`（`admin/employees/actions.ts`。`editWageRate`/`deleteWageRate`と同じ、
  適用開始日変更時は一意制約の衝突確認→旧行削除→再作成のパターン）。
