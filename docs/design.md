...
app/
  ...
  admin/                 管理者向け（レスポンシブ。md以上は左サイドバー/スマホは下部タブナビ、ネイビー）
    ...
    daily/               日当設定（page/ui/actions）。給与期間ごとの「従業員×日ごと」の勤務時間・支給額一覧。
                         ...
                         昼食補助が当日だけ上書きされている行は⚠️マーク→タップで理由を吹き出し表示（§17.6）
    close/               締め処理 + 税理士資料を統合（プレビュー・締め・支払済み・明細メール配信）。
                         タイトルは省略、期間は「締め日：{終了日}、支払日 {支払日}」の1行。操作ボタンはヘッダ部に配置。
                         締め済みは 1行目=締め解除/支払済みにする、2行目=明細をメール配信(アイコン+「従業員」)/
                         税理士へメール(アイコン+「税理士」)/PDF/CSV(後2つは文字ラベルのボタン)。明細配信は0円明細を宛先除外。
                         ⚠️ 2行目のラベルは「〜へ」を付けない(2026-09-11)。iPhone標準幅(390px)でCSVまで
                         1行に収めるため。aria-label/title には「従業員へ明細をメール配信」等の説明を残している。
                         ...
                         月度途中で時給が変わった従業員は時給ごとに行分割（時給非依存の列はrowSpanで人単位にまとめる。§18）
                         明細は**1人おきの縞模様**（`zebraRowClass`。時給分割で複数行になる人も1色でまとめる）。
    ...
    tax-table/           源泉徴収税額表(月額表)専用ページ(メニュー「税額表」。2026-08-22、設定画面から独立)。
                         ...
                         「この内容で取り込む」)と、`<details>`で折りたたんだ手動貼り付けの予備経路
                         （Excelからのタブ区切り貼付・桁区切りカンマ除去等に対応）を持つ。年度ごとに
                         取り込み日時を表形式で表示。詳細は「乙欄・課税対象額88,000〜105,000円の
                         従業員が源泉所得税¥0になるバグ」の項の「🆕 Excelファイルからの自動入力」参照
...
- `computeIncomeTax()`: 源泉所得税。
  - 課税対象額（基本給+深夜勤務手当+残業手当+昼食補助。**交通費は非課税のため含めない**。国税庁の
    月額表の基準額「その月の社会保険料等控除後の給与等の金額」も非課税の通勤手当を含まない額であり、
    この扱いで一致する）を `withholding_tax_table`（設定画面から貼付取込。形式は国税庁公開様式に準拠:
    以上,未満,甲0〜甲7,乙。乙欄のみ3列も可）で参照する。甲欄は扶養0〜7人まで参照
    （`Math.min(dependents,7)`）。課税対象額が税額表の登録範囲の**上限を超えている**場合はエラーで
    締めを止める（誤計算防止）。
    - 取込時、国税庁月額表の先頭にある「(最小額)円未満→0」の変則行（未満欄が空で「以上」に最小額が
      入る行）は**取り込み対象外**（上限なしの正当な行は最終行=最大の「以上」のみ）。その帯（=**表の
      最小「以上」金額未満**。年度の税制改正で変わりうる。**令和8年分は105,000円**）は
      `computeIncomeTax` がハードコードではなく取り込み済みデータの最小`min_amount`から動的に判定し、
      **甲欄は0円、乙欄は課税対象額×3.063%(1円未満切り捨て)**とする（表に1件もデータが無い場合のみ
      暫定的に88,000円を使う）。
    - 🔴 **バグ修正（2026-08-21、オーナー報告）**: 以前はこの帯を「甲欄・乙欄とも一律0円」としていた
      （実装ミス）。国税庁の月額表は先頭行でも**乙欄だけは0円ではなく3.063%**であり、令和8年分で
      表の最小額が88,000円→105,000円に上がったことで、乙欄かつ課税対象額88,000〜105,000円の
      従業員（例: 課税対象額103,000円）の源泉所得税が誤って¥0と表示される実例が発生した
      （過去に確定済みの給与明細にはこの帯に該当した例が無いことをSQLで確認済み。実害は今回が初）。
      `lib/payroll.ts`の`computeIncomeTax()`を、ハードコードされた`88000`判定を廃し**税額表データの
      最小`min_amount`から動的に「表の下限」を導出**した上で、その下限未満は甲欄0円・乙欄3.063%を
      一貫して適用する形に修正した。`lib/payroll.test.ts`に修正後の期待値でテスト追加（幸田さんの
      実例=課税対象額103,000円・乙欄・扶養0人 → ¥3,154 も回帰テストとして追加）。
  - 国税庁からの自動取得は非対応（NTAは月額表をPDF/Excelでのみ公開しており安定した機械可読源が無く、
    当環境からnta.go.jpはネットワーク遮断のため）。年に1度、国税庁の月額表を貼り付けて取り込む運用。
  - 🆕 **Excelファイルからの自動入力（2026-08-22追加）**: 上記バグの原因調査で「手動コピペでは
    列・行の選択ミスが起きうる」ことが分かったため、国税庁の月額表Excel（.xls/.xlsx）を
    そのまま選択するだけで対象年・貼付欄を自動入力できる機能を追加した。
    - `src/lib/tax-table-excel.ts`の`parseTaxTableExcel()`（純粋関数・SheetJS `xlsx`使用）が
      シートを走査し、**「以上」列が数値でない行（見出し・空行・冒頭の「(最小額)円未満」の
      文言行）は読み飛ばし**、「以上・未満・乙欄がすべて数値」の帯だけを抽出する。「未満が空だが
      乙欄は数値」の行（＝表の最終行、令和8年分は¥740,000）に達したら**そこで読み取りを打ち切る**
      （それ以降は超高額所得者向けの税率計算式に切り替わり、本アプリの時給制アルバイトでは
      到達しないため）。タイトル行から「令和N年」を検出し西暦へ変換（全角数字にも対応）。
    - 実装したロジックを**オーナー実際の令和8年分ファイルで検証**: 抽出結果232行と、既存DBの
      232行を`min_amount`昇順でCSV化してMD5比較し**完全一致**することを確認済み（調査時のみ、
      本体コードには影響しない検証作業）。
    - 当初は`admin/settings/ui.tsx`の`TaxTableForm`に「ファイルを選択→貼付用テキストエリアを
      自動入力→既存の『取り込む』ボタンを押す」形で追加したが、**生のCSVテキストエリアが主要な
      見た目のままだったためオーナーには「結局コピペのまま」に見えてしまった**（2026-08-22指摘）。
      **UIを「アップロードが主・貼り付けは予備」に再設計**: ファイル選択ボタン(青・破線枠で
      目立たせる)を押すとその場でクライアント側解析→「年・区分数・範囲」だけを要約したプレビュー
      カードを表示→専用の「この内容で取り込む」ボタンで送信、という流れにし、**生のCSVは画面に
      出さない**（`importTaxTable`へはコンポーネント内で`FormData`を組み立てて直接渡す。textarea
      DOMには依存しない）。手動貼り付け(従来のtextarea方式)は`<details>`で**既定折りたたみ**の
      予備経路として残した。`xlsx`は選択時に**動的import**（この画面でしか使わないため他画面の
      バンドルに含めない）。自動送信はせずプレビュー確認後にボタンを押す運用は維持（税務データの
      ため）。読み取り失敗時はエラーメッセージを出し、折りたたみの手動貼り付けへ誘導する。
    - ⚠️ **`xlsx`(SheetJS)のnpm公開版には修正版が無い既知の脆弱性(プロトタイプ汚染・ReDoS)がある**
      （`npm audit`で検出。SheetJS社は自社CDN配布版でのみ修正しているためnpm audit fixでは直らない）。
      オーナーと協議のうえ、**管理者自身が選んだ信頼できるローカルファイルをブラウザ内で読むだけ**
      （外部・不特定多数からのアップロードを処理するものではない）という利用形態のリスクは
      許容範囲と判断し採用した。代替案（`exceljs`。既知の重大脆弱性なしだが .xls 非対応）も
      検討したが、国税庁配布ファイルが.xls形式だったため見送った。
    - テスト: `lib/tax-table-excel.test.ts`に2件（実物相当の構造を模した合成ワークブックで
      抽出結果を検証、データ0件時にエラーを投げること）。実ファイルでの検証は上記の通り別途実施済み。
    - 🆕 **設定画面から独立し専用メニュー「税額表」に（2026-08-22、オーナー依頼）**: 頻度・重要度が
      高い機能のため`admin/settings/`から`admin/tax-table/`へ切り出した（`TaxTableForm`/
      `TaxTableRow`ごと移動。`admin/settings/page.tsx`からは`withholding_tax_table`のクエリごと
      削除）。`admin/nav.tsx`の`moreLinks`に`TableIcon`で追加（PCサイドバー「管理」グループ・
      スマホ「その他」ハンバーガーの「設定」の直後）。`importTaxTable`
      （`admin/settings/actions.ts`に残置。他の設定系アクションと同じファイルで管理）の
      `revalidatePath`も`/admin/settings`→`/admin/tax-table`に更新。
    - 🆕 **「登録済みデータ」表に、表には現れないルールの行を追加（2026-08-22、オーナー指摘）**:
      最小「以上」金額未満（＝Excelの先頭にある「(最小額)円未満」の変則行。取り込み対象外のため
      DBには保持していない）は`computeIncomeTax()`が動的に「甲欄0円・乙欄は課税対象額の3.063%」と
      判定するが、これは表の外の暗黙のルールで、登録済みデータの一覧を見ただけでは分からなかった。
      `admin/tax-table/ui.tsx`の一覧の**先頭行**（アンバー色で区別）に
      `{表示中の年の最小min_amount}円未満: 甲欄はすべて0円 / 乙欄は課税対象額の3.063%`
      を表示するようにし、DB上のデータと実際に適用される全ルールが一覧だけで分かるようにした。
    - 🆕 **画面冒頭の説明文が「88,000円」を直書きしていて矛盾（2026-08-22、オーナー指摘）**:
      `computeIncomeTax()`の閾値ハードコード撤去（本節冒頭のバグ修正）・Excel取り込み・上記の
      表外ルール表示、と一貫して「表の最小登録額は年度により変わりうる・動的に扱う」よう直したのに、
      画面冒頭の説明文だけが「課税対象額が月88,000円以上の人がいる場合に必要です(88,000円未満は
      自動計算: 乙欄3.063%・甲欄0円)」と**具体的な金額(88,000円)を直書き**したままで矛盾していた。
      「課税対象額が下の表の最小登録額以上の人がいる場合に必要です(最小登録額未満は下の一覧に
      表示のとおり自動計算されます)」に修正し、具体的な金額は上記の一覧先頭行のみが示す形に統一した。
    - 🆕 **`computeIncomeTax()`のエラーメッセージも旧画面名のままだった（2026-08-22、ドキュメント
      監査で発覚）**: 課税対象額が税額表の登録範囲の上限を超えたときの`PayrollError`メッセージが
      「設定画面から税額表を登録してください」のままで、税額表画面が独立した後も更新されていな
      かった（案内先を誤らせる）。「『税額表』画面から登録してください」に修正した
      （`src/lib/payroll.ts`）。
...
- 🔴 **`emailPayslips` は `employees.status` を確認していなかった（2026-08-26発覚・同日修正）**:
  ...
  （本番で退職済みの太郎ちゃんに実際に送信されてしまい発覚）。クエリに `status` を追加し、
  `allTargets` のフィルタに `status !== "active"` の除外を追加して修正。給与明細画面自体の表示は
  変更していない。他のメール配信（お知らせ配信・招待・税理士向け）は元々 `status='active'` で
  絞られていることを確認済み（`docs/handover.md`「テスト用従業員『太郎ちゃん』の扱い」参照）。
...
- **テスト通知**（2026-09-18追加・管理者/従業員共通）: 登録済みの端末では「この端末にテスト通知を送る」
  ボタンが出る。サーバーアクション `sendTestPushToThisDevice(endpoint)`（`app/account/actions.ts`）が
  **押した端末の購読1件だけ**に送る（RLS により自分の購読しか読めないので他人には送れない）。
  404/410（失効）が返ったらその購読を削除し、登録し直すよう案内する。
  🔴 **送信成功＝通知が出る、ではない**。Apple の Web Push は購読が実質無効でも 2xx を返し続けることがあり
  （2026-09-18に発生。§24.5 の実機確認の節・引継書参照）、失効として検出できない。最終確認は
  「端末に通知が見えたか」なので、成功メッセージにも登録し直しの手順を書いている。
- **Service Worker**: `push` / `notificationclick` を追加。**`fetch` は依然として横取りしない**
  ので、ナビゲーションへの影響は無い（下記の重要な教訓を参照）。

### 🔔 シフト開始前・終了前後の通知（従業員向け・Web Push・2026-09-26追加）
...
- **終了の通知（2026-09-26追加）**: 同じ欄の「シフトの終了時間の N 分前に通知する」（-720〜720分、
  `end_minutes_before`）。**マイナスは終了の N 分後**（退勤打刻の忘れ防止）。出勤打刻済み・退勤打刻がまだの日だけ、
  予定時刻から10分以内に限って送る（設定変更の直後に何時間も前のシフトへ飛ばさないため）。
  開始・終了は別々にオン/オフでき、両方オフなら行を消す。文面は `src/lib/shift-reminder-notify.ts`。
- **重複防止**: `shift_reminders`（従業員, 業務日, 種類 start/end, 基準日時 `target_at`）。通知後にシフト時刻が変わったら新しい時刻で再通知。
...
### 7.8 「毎回ログインを求められる」「打刻画面が出ない」問い合わせへの案内（AccessHelp・2026-08-31追記）
- ログイン画面（QR経由=`?redirect=/clock…`のとき）と打刻確認画面の下部に、共通コンポーネント
  `src/components/AccessHelp.tsx` が案内文を表示する。**プライベートブラウズ／Cookieブロック**を疑わせる文言と、
  `navigator.cookieEnabled===false` を検知したときの赤字警告を出す（プライベートブラウズ自体はJSから確実に
  判定できないため文言での注意喚起にとどめる）。
- 🔴 **実際の問い合わせ事例で判明した、もう一つの原因（新規利用開始者への注意事項）**: 職場のQRを
  **純正「カメラ」アプリ以外**（LINEやその他アプリに内蔵されたQRスキャナー等）で読み取ると、
  多くの場合そのアプリ内の「アプリ内ブラウザ」（iOSの`SFSafariViewController`。Safari本体ではなくWebKitを
  埋め込んだ別画面）でページが開く。これはSafari本体と見た目が似ているため気づかれにくいが、
  **画面下部に「外部ブラウザで開く」アイコン（四角＋矢印/コンパスのようなアイコン。共有・更新アイコンと並ぶ）が
  あるのが見分け方**（Safari本体にはこのボタンは存在しない）。アプリ内ブラウザはアプリによってセッション/Cookieの
  扱いが異なり、毎回ログインを要求される・打刻画面までたどり着けない、といった症状の原因になり得る
  （2026-08-30に実際の従業員からの問い合わせで確認。送られてきたスクリーンショットのフッタに
  この「外部ブラウザで開く」アイコンが写っていたことから特定）。
  - **新規利用開始者への案内ポイント**: 職場QRは必ず**純正「カメラ」アプリ**で読み取ること。もし他アプリ経由で
    アプリ内ブラウザとして開いてしまった場合は、その「外部ブラウザで開く」アイコンをタップして実際のSafariで
    開き直してから使う（そのうえで上記のプライベートブラウズ／Cookie設定も確認）。
  - この判定はUser-Agentのみでは行っていない（`AccessHelp.tsx`は`navigator.cookieEnabled`のみ検知）。
    アプリ内ブラウザかどうかの自動検知・専用の案内文は**未実装**（現状は人力サポート時の判断材料として
    この設計書に記録するのみ）。将来UIに組み込む場合はこの節を起点に検討すること。
...
### 10.3 営業カレンダー(外部サービス・参照のみ)への導線（2026-07-24追加。2026-09-18に切替済み）
> 🔴 **2026-09-18: サイドバー「関連情報 > 営業カレンダー」のリンク先をアプリ内の `/calendar/embed`（§24.4）に
> 変更した（PC/iPadの縦サイドバーとスマホのメニューの両方）。以下は切替前の記録。**
> **その後、管理者ナビからはこの項目を削除**（「管理 > 営業カレンダー」と同名の項目が2つ並んで紛らわしいとオーナー指摘。
> 管理者が公開ページを見るときは編集画面の 👁 アイコンから）。**従業員ナビには残す**（従業員にはこれが唯一の導線）。
> 旧アプリ（`oominami-calendar`）は1ヶ月ほど様子を見てから停止する。
...
- **「退職者を含む」トグル（管理者のみ・既定オフ、2026-08-20追加）**: 元々この画面は
  `employees.status`で絞っていなかった（2026-08-02、実在の退職者の過去実績も参照できるべき
  という判断）が、2026-08-20に7月分実績を「退職者として複製した仮の従業員」へ付け替える運用
  （`docs/handover.md`「7月分実績の退職者複製案」）を実施したところ、その**仮の退職者**が
  一覧に混ざって紛らわしいという指摘があった。対応として`loadDailyReport(from, to, employeeId?,
  includeRetired?)`に第4引数を追加し、**既定は`status='active'`のみ**（退職者を除外）、管理画面
  上部のトグルボタン（`?retired=1`）でオンにすると退職者(仮の退職者複製を含む)も表示する方式に
  変更した。従業員自身の画面（`(employee)/daily/page.tsx`）は常に`includeRetired={false}`固定
  （退職済みは`requireEmployee()`でログイン自体ができないため実質影響なし）。CSVダウンロード
  （`buildDailyReportCsv`）もトグルの状態を引き継ぐ。PDFは画面のキャプチャのためトグルの状態が
  自動的に反映される。
...
### 11.7 給与計算プレビューは締め前は常に「現時点の暫定額」を表示・課税対象額列の追加（2026-08-21）
...
- **`computePayslip`に`ignoreIncomplete?: boolean`を追加**（`lib/payroll.ts`）。`true`のときは退勤未入力の
  ...
  （退勤未入力を見逃したまま確定させないため。この仕組みはあくまでプレビューの表示のみに関わり、
  確定処理には一切影響しない）。
- **`calculatePeriodPayroll`（`lib/payroll-data.ts`）に`opts?: { ignoreIncomplete?: boolean }`を追加**し、
  `computePayslip`へそのまま渡す。
- **UI**: 当初はトグルボタンで切り替える設計（`?tentative=1`）で実装したが、オーナーから
  「締め前は常に現時点の最新の勤務実績で計算されていればよく、トグルでの切替は不要」との要望を受け、
...
- **課税対象額列を追加（同日、オーナー依頼）**: 「総支給」列の直後（＝「所得税」列の直前）に
  ...
  を表示し、源泉徴収税額表と突き合わせて検算しやすくした（`computeIncomeTax()`の項の
  「🔴 バグ修正（2026-08-21、オーナー報告）」参照。このバグ調査の過程でオーナーから要望されたもの）。
- **「交通費」列に非課税の注記を追加（2026-08-22、オーナー依頼）**: 交通費が課税対象額に含まれない
  ことを表内で明示するため、列見出しを「交通費*」にし、表の右肩（`overflow-x-auto`の直上、
  右寄せ）に「*印は課税対象外」を追加した。`print-report`の対象内なのでPDF/印刷にも表示される。
...
## 16. 起動時スプラッシュ・空白画面の短縮（2026-08-26追加）
オーナー報告「久しぶりにアプリを開くと数秒間真っ白になる」への対応。実機の画面録画を
...
- Next.js の `loading.tsx` は、**同じセグメントの`layout.tsx`自体の待ち時間はカバーしない**
  （`page.tsx`と、その下の子だけをSuspenseでラップする）。`admin/layout.tsx`・
  `(employee)/layout.tsx`はどちらも`requireAdmin()`/`requireEmployee()`で認証確認・DB問い合わせを
  行っているが、これらのlayout自身の待ち時間をカバーするのは**親セグメント（ルート）の
  `loading.tsx`**になる。そのため`src/app/loading.tsx`（ルート）を新設していなかった時期は、
  layoutの認証確認中は本当に何も表示されない空白になっていた。
- **`redirect()`を呼ぶページは、その待ち時間を`loading.tsx`で一切カバーできない**（構造上の制約）。
  HTTPリダイレクトはボディを送れないため、Next.jsは`redirect()`を呼ぶ可能性がある非同期
  Server Componentの実行を**完全に完了させてから**でなければストリーミングを開始できない。
  旧`src/app/page.tsx`（"/"）は`auth.getUser()`+`employees`問い合わせの後に必ず`redirect()`を
  呼ぶ構造だったため、ここでの待ち時間はどんなローディングUIを足しても表示できなかった。

### 16.3 middlewareでの二重問い合わせ解消（2026-08-26）
- 画面録画で確認したところ、**認証済みユーザーが"/"（PWAの`start_url`）を開くたびに、
  `middleware.ts`と`src/app/page.tsx`の両方で`auth.getUser()`が実行され、さらに`page.tsx`側で
  `employees.is_admin`の問い合わせも行っていた**（`middleware.ts`は元々全ページで
  `auth.getUser()`を実行しセッションをrefreshする役割だったため、`page.tsx`側の同じ確認は
  完全な重複だった）。この重複区間は前段16.2のとおりローディングUIで一切カバーできないため、
  そのまま体感の空白時間になっていた。
- **対応**: `src/lib/supabase/middleware.ts`の`updateSession()`に、`user`が存在し
  `pathname === "/"`のときは`employees.is_admin`まで問い合わせて`/admin`または`/timesheet`へ
  直接`NextResponse.redirect()`する分岐を追加。これにより`page.tsx`側での再確認が不要になり
  （認証済みユーザーは常にmiddleware側で先に振り分けられるため`page.tsx`のロジックは実質
  到達しなくなる。未認証ユーザー向けの元のロジックは`page.tsx`にそのまま残置＝保険）、
  問い合わせ回数を3回→2回に削減した。
- **既知の限界（これ以上は現状据え置き。オーナー合意・2026-08-26）**: 上記の対応後も、
  **Cloudflare Workers Freeプランのコールドスタート＋Supabaseとの通信**そのものにかかる
  数百ms〜数秒はローディングUIでは短縮できない（middlewareはReactのレンダリング開始前に
  動くため、ここでも`Splash`は出せない）。さらに縮めるには`middleware.ts`の`auth.getUser()`
  （毎回Supabaseに通信してJWTを検証している）を、非対称鍵によるJWTのローカル検証
...
### 17.6 日別画面での当日変更の可視化（2026-09-04追加）
**背景**: 日別画面（§11）の一覧表だけを見ても、その日の昼食補助額が通常どおりなのか当日だけ上書き
されたもの（現物支給など）なのか区別できず、変更理由を確認するには勤務表画面まで開く必要があった。
「日別画面の一覧表で変更箇所に気付けるようにしたい」というオーナー要望に対応。

- 日別画面（`admin/daily`・従業員用`/daily`。両者は`DailyReportView`（`admin/daily/report-view.tsx`）を
  共用しているため1箇所の修正で両画面に反映される）の昼食補助セルに、その行の`work_entries.lunch_change_amount`
  が非nullの日だけ⚠️マークを表示する。タップ/クリックで理由（`in_kind`=現物支給／`other`=その他+自由入力。
  ラベルは勤務表画面§17.4と同じ`LUNCH_REASON_LABELS`）を吹き出しで表示する。従業員・管理者どちらの画面でも
  同じ挙動で、読み取り専用（編集はできない。§17.4の「透明性のための表示のみ」方針を踏襲）。
- `lib/daily-report.ts`の`DailyRow`に`lunchOverridden`（真偽値）・`lunchReasonType`・`lunchReasonNote`を追加。
  entriesクエリに`lunch_change_reason_type`/`lunch_change_reason_note`を追加取得し、`lunch_change_amount`が
  非nullの行だけこれらを設定する（DBのCHECK制約により金額があれば理由区分は必ず入っている）。
  退勤未入力等でエラー表示になる行（列自体をまとめてcolSpanで潰す行）は対象外。
- 吹き出し本体は`admin/daily/ui.tsx`の`LunchReasonBadge`（新規クライアントコンポーネント）。日別画面の
  テーブルは横スクロールする`overflow-x-auto`の中にあるため、`absolute`ではなく**`position: fixed`で
  クリック位置（`getBoundingClientRect()`）を起点に描画**し、スクロールコンテナによるクリップを回避
  （画面幅からはみ出さないよう左位置をクランプ）。透明な全画面オーバーレイを重ねて外側タップで閉じる。
- 実装ファイル: `src/lib/daily-report.ts` / `src/app/admin/daily/{report-view,ui}.tsx`。

## 18. 締め画面：月度途中で時給が変わった場合の明細行分割（2026-09-01追加）

**背景**: 月の途中で時給が変わった（昇給など）従業員がいても、締め画面の明細表は期間末時点の1つの時給だけで
基本給・深夜手当・残業手当を表示していたため、実際の内訳と食い違って見えていた（時給変更への対応自体は
給与計算では既に日ごとに`effectiveAt()`で正しく行われており、表示だけの問題）。

- `lib/payroll.ts`の`computePayslip()`に`wage_breakdown: WageBreakdown[]`
  （`hourly_wage, work_days, total_minutes, night_minutes, overtime_minutes, base_pay, night_pay,
  overtime_pay`）を追加。勤務日を日付順に処理しながら、日ごとに有効な時給（`effectiveAt()`）でグルーピング
  して積み上げる。時給が1種類のみの期間なら要素数1になり、従来と同じ表示のまま。
- `admin/close/page.tsx`の表は`payrolls.flatMap()`に変更し、1従業員につき`wage_breakdown`の要素数ぶんの行を
  出力する。**時給に依存する列**（日数・実働・深夜・残業・時給・基本給・深夜手当・残業手当）は行ごとに
  分け、**時給に依存しない列**（氏名・交通費・昼食補助・総支給・課税対象額・所得税・前払金・差引支給）は
...
- 表の縦位置は**全列そろえて既定（中央）**にする。`align-top` は付けない（氏名列も含む）。
  PDFボタンで行が少し高くなるため、一部だけ `align-top` だと上にずれて見える（2026-09-11）。
...
### 20.2 プレビュー → ダウンロード / 共有（全PDFボタン共通）

- 「PDF」を押すといきなりダウンロードするのではなく、**プレビューのダイアログを開き**、そこから
  出力方法を選ぶ。**アプリ内のPDFボタンはすべてこの方式に統一**している（2026-09-11）。
  共通部品は `src/components/PdfPreviewDialog.tsx`。適用先は
  **給与明細（従業員別）/ 給与明細の一覧表 / 日別実績 / 出退勤QRコード**の4か所。
- **プレビューには「出来上がったPDFのページ画像そのもの」を出す。** 元のDOMを縮小して見せる方式だと
  「プレビューでは正しいのにPDFは崩れている」という食い違いが起こりうる（§20.3 の事故が実例）。
  画像なら必ず一致する。ページ画像は幅1000px上限のJPEGに落として持つ（`toPreviewImage`）。
  キャプチャ関数は `PdfResult = { blob, pages }` を返す。
- **出し分け**:
  - **スマホ・タブレット**: 「共有」だけを主ボタン（ブルー）で出す。ダウンロードしても扱いにくいため。
  - **PC**: 「ダウンロード」を主ボタン（ブルー）にし、共有できる端末なら「共有」も副ボタン（枠線）で併置。
  - 🔴 **「ダウンロード」を隠す条件に `navigator.canShare({files})` を使わないこと。**
    **macOS の Safari/Chrome も真になる**ため、PCでもダウンロードが消える（2026-09-11に発生）。
    スマホ判定は UA（`iPhone|iPad|iPod|Android`）＋ iPadOS が Mac を名乗る分をタッチ点数で拾う
    `isMobileDevice()` で行い、`canSharePdf()`（共有ボタンを出すか）とは**別の判定**として持つ。
- ⚠️ **PDFはダイアログを開いた時点で先に作る**。「共有」を押してから作ると、iOS では
  `navigator.share()` が「ユーザー操作から直接呼ばれていない」と見なされて弾かれる
  （`share()` の中で `await` を挟まないこと）。作り終わるまでボタンは `disabled`。
- ダイアログは `createPortal` で **body 直下**に出す。表（`overflow-x:auto` の枠）の中に置いたままだと、
  祖先に `transform` 等が付いたときに `position:fixed` の基準がずれて隠れうるため。
- ⚠️ **キャプチャ方式は用途ごとに違うまま**にしてある（`make` を呼び出し側が渡す）。
  一覧表・日別＝`captureElementToPdfBlob`（html2canvas-pro / 横向き / Tailwind）、
  給与明細＝`captureSheetToPdfBlob`（本家 html2canvas / 縦向き / 自己完結CSS）、
  QRシート＝`clock.tsx` 内の独自処理（本家 html2canvas / `.qr-print-sheet`）。
  **「1つにまとめよう」とすると壊れる**（§20.3、および clock.tsx のコメント参照）。
...
- 右上に**支払元2行 + 印**（要望どおりの配置）。支払元のかたまりは見出しより**1行ぶん下げ**
  （`.pslip-issuer { margin-top:6mm }`）、**1行目だけ大きめ・太字**にする（`.pslip-issuer-line1`）。
- 左右の余白は **24mm**（`.pslip-sheet` の padding）。上下より広く取っているのは、A4幅いっぱいだと
  項目名と金額が左右に離れすぎて読みにくいため（2026-09-11、オーナー指摘）。
- 見出し「給与明細書」と、その下に**大きめ・太字の「◯年◯月度」**（`.pslip-period`）、氏名（「様」付き）、対象期間・支払日。
...
## 21. 一覧表の縞模様（2026-09-11追加）

行数の多い表で横に目が滑らないよう、明細行を**1行おきの縞模様**にした（オーナー依頼）。

- 共通ヘルパー `zebraRowClass(index)`（`src/lib/table.ts`）。偶数=`bg-white` / 奇数=`bg-gray-100`。
  当初 `bg-gray-50`（#f9fafb）にしたが「薄くて視認しにくい」との指摘があり、
  `bg-gray-100`（#f3f4f6）へ一段濃くした（2026-09-11。白との明度差が約2.2倍になる）。
- ⚠️ **`tr:nth-child(even)` のようなCSSではなく引数で番号を渡す形**にしている。
  給与明細画面は**1人が複数行（時給ごとの行分割）になる**ため、そこでは「行番号」ではなく
  **「従業員の番号」**を渡して同じ人の行を1色にまとめる必要がある。
- 🔴 給与明細画面では、その番号は **`payrolls` の添字ではなく「実際に行が出る従業員の通し番号」**
  （`rendered` 配列の添字）を使う。**その期間に勤務実績が1日も無い従業員は `wage_breakdown` が空で
  1行も描画されない**ため、`payrolls` の添字で数えると飛ばされたぶん前後の行が同じ色になる
  （2026-09-11に発生。勤務0件の従業員が2人の間に挟まっていた）。
  なお「勤務0件の従業員が一覧に出ない」のは以前からの挙動で、縞模様を入れて初めて可視化された。
- ⚠️ **横スクロールで固定している列（`sticky`）のセルは `bg-inherit` にすること。**
  `bg-white` を直接指定したままだと、固定列だけ白で残って縞から浮く。
- 適用先: 給与明細（`admin/close`）/ 日別実績（`admin/daily/report-view`）/ 従業員一覧
  （`admin/employees/ui`）/ 操作ログ（`admin/logs/LogsView`）/ 税額表（`admin/tax-table/ui`）。
...
- **「泊まり可」は営業時間の補足事項なので、枠・背景を付けず濃いグレー（`#374151`）の文字だけで出す**
  （公開カレンダーのセル・吹き出し、管理画面、ポスターで統一。2026-09-18、オーナー指示で金色の枠付きチップから変更）。
...
- `/admin/calendar`（サイドバー「管理」→「営業カレンダー」）: `admin/calendar/page.tsx`・`ui.tsx`・`actions.ts`。
  ...
  - 月グリッド `components/business-calendar/MonthCalendar.tsx`（管理画面用。手修正の日に●、左右スワイプで月移動）。
    **イベント枠（単日・複数日）は左右を片側2px空ける**（営業時間の枠は1px。隣の枠との区切りを見やすくするため。2026-09-23）。
    ...
    （`text-base sm:text-lg` 太字）。**月の状態は曜日行の色ではなく状態バッジだけで示す**
    （2026-09-18。以前は準備中=薄イエロー／公開中=薄グリーンだった）。
...
- `/calendar/embed`（ログイン不要。middleware の公開パスに追加）: `app/calendar/embed/`。旧 `oominami-calendar` の
  ...
  - **高さの自動調整**（2026-09-18追加）: 中身が iframe より高いとHP側に内側のスクロールバーが出る
    （760px 固定に対し、泊まり可の行が入る月は 900px を超える）。埋め込みページが body の高さを
...
操作説明の動画や資料の一覧。管理者が設定画面で登録し、メニュー「アプリの解説」から開く。
各項目は「🎬 動画をアプリに保存」か「🔗 URL（外部の資料・動画へのリンク）」のどちらか。動画はアプリ内の画面いっぱいのプレイヤーで再生する。
...
- Storage `app-guides`（非公開・1ファイル50MB・`video/mp4`/`video/quicktime`）: 閲覧は管理者は全部、従業員は
  `for_employee` の解説の `video_path` に一致するものだけ。アップロード・更新・削除は管理者のみ。
- 従業員／管理者それぞれで読める行・動画が正しいことを、DB 上でロールを切り替えて確認済み（テスト行はロールバック）。

### 26.2 登録（設定画面の最後「アプリの解説」。`admin/settings/guides.tsx`、アンカー `#app-guides`）
- 一覧は**1項目1行の表**: 公開対象｜🎬/🔗 タイトル｜解説の冒頭（1行で省略）｜↑↓編集。明細は縞模様（`zebraRowClass`）。
...
- **日時はどちらも自動**（手入力の欄は無い。2026-09-26 オーナー依頼）。
  - 動画: 選んだ時点で MP4/MOV の `moov > mvhd` の creation_time を読む（`lib/mp4-meta.ts` の `readMp4CreationTime`。
    ボックスの見出しと mvhd の先頭だけを `slice` で読むので大きな動画でも軽い。テスト `lib/mp4-meta.test.ts`）。
    iPhone・カメラの動画には入っている。記録が無い（0）動画はファイルの更新日時（`File.lastModified`）を使い、画面にその旨を出す。
...
- サーバーアクション（`admin/settings/actions.ts`）: `saveAppGuide` / `deleteAppGuide` / `moveAppGuide` / `discardGuideVideo`。操作ログに記録。

### 26.3 表示・再生
- 一覧: 管理者 `/admin/guides`（`for_admin` の行）、従業員 `/guides`（`for_employee` の行）。共通部品 `components/AppGuideList.tsx`。
  各行はタイトルと解説だけ（冒頭の説明文・「▶ 動画を見る」・公開対象の表示はオーナー依頼で削除）。
  動画の項目は同じタブで再生画面へ、URL の項目は別タブで開く（URL の項目だけ「Google ドライブで開く ↗」などを添える。`linkKind`）。
- 再生画面 `/watch/[id]`（`app/watch/[id]/page.tsx`・`player.tsx`。管理者・従業員共用で、メニューの枠を出さない）:
  黒背景に `<video controls playsInline preload="metadata">` を画面いっぱい `object-contain`。署名付きURL（4時間）。
  横向きのスマホ（高さ500px以下の横長）では見出し・概略を隠し、左上に×だけ重ねる。「⛶ 全画面」は iPhone では `webkitEnterFullscreen`
  （標準プレイヤー）、それ以外は `requestFullscreen`。「閉じる」は勤務ルール画面と同じ（アプリ内から来たら戻る、それ以外は `/`）。
  SW は fetch を横取りしないので、範囲リクエスト（シーク）もそのまま Supabase Storage に届く。見られる項目は RLS で決まる。
- メニュー: 管理者・従業員とも、PCサイドバーは「関連情報」グループの最後、スマホは「その他」の最後（ログアウトの上）。
  その画面を開いているときは「関連情報」を開いた状態にし、スマホの「その他」を選択中の色にする。
- 経緯: 最初は Google ドライブの共有リンクを登録していたが、iPhone（特にホーム画面に追加したアプリ）では小さいプレビューにしかならず、
  Google にログインしていない状態では「何らかの問題により、この動画を再生できませんでした」になった（ドライブ側の制限でアプリからは直せない）。
...
アプリ登録用の書き出しまで1回で作り直せる（2026-09-26 に2本目で作り直して、前回の完成品と長さ・音量・映像が一致することを確認）。
- 映像: 1920×1080 の HTML アニメーション（時刻 t を渡すと1コマを描く `renderAt(t)`）を、puppeteer-core（ローカルの Chrome）で
  30fps で1コマずつ撮影し、ffmpeg（`ffmpeg-static`）で H.264 に。画面は実際の画面の文言・配色を再現した模型（スマホ枠に表示）。
- ナレーション: VOICEVOX エンジン（公式配布の macOS arm64 版）の「ずんだもん（ノーマル）」。話速1.12・抑揚1.15。
  各文を先に合成し、**文の長さに合わせて映像側の時間を区間ごとに伸ばす**（映像の時刻 ⇔ 実時間の対応表を作る）。
  - ユーザー辞書: 「勤務時間」＝キンムジカン（アクセント3。「営業時間」と同じ形）、「月末」＝ゲツマツ（0）。
  - 無声化した「ヒ」（例:「通しの日と」が「人」に聞こえる）は、音声合成の問い合わせ結果で母音を有声に書き換える。
  - 「10月末」は「じゅうげつまつ」になるため、原稿は「10月の月末」と書く。
  - 🔴 VOICEVOX の利用規約により、動画内に「**VOICEVOX:ずんだもん**」のクレジット表記が必要（まとめ画面の右下に入れている）。
- BGM: 自作の合成音（外部音源なし・112BPM）。ナレーション中は自動で音量を下げる（sidechain）。全体 −16 LUFS 前後。
- アプリ登録用: `-crf 27 -tune animation` で軽量化（約1/3）し、`-metadata creation_time=<書き出し日時(UTC)>` を付ける（§26.2 の作成日時用）。
- 現在の動画（2026-09-26）:
  - 「営業カレンダーの設定方法」（3分7秒）: しくみ（HPのカレンダーは給与計算アプリがルールどおり自動作成 → 管理するのはいつもと違う日だけ）→
    メニューからの開き方 → 毎月の流れ → 画面の見方 → 臨時休業・時間変更・イベント・元に戻す → 月の注釈 →
    「営業時間そのものが変わるときは『営業時間と勤務時間の管理』を参照」→ ホームページへの反映 → まとめ（10月時点の運用例）。
  - 「営業時間と勤務時間の設定」（2分34秒）: 画面の役割 → 適用開始日とは → 変わるときの操作（例: 2027/4/1 から開店・早番・昼休憩を1時間遅らせる）→ 勤務ルールへの反映。
  - 「シフトを自分のカレンダーに表示する」（従業員向け・2分12秒。`movies/shift-calendar`）: できること（調整中は「仮:」）→ 開き方（右上の名前 → アカウント設定 →
    カレンダー連携。パソコンは左下の名前）→ iPhone（「検索」→「✓」）→ Google（パソコンで「追加」。名前は webcal://… から自動で変わる）→ 注意（反映の遅れ・URLを教えない）。§22 の画面。
