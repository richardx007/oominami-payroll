...
- 判定は基準座標からの距離が許容半径超なら「圏外」。`警告のみ`なら記録して管理者ログ/一覧で確認、
  ...
  - **操作ログの分類**（2026-07-19追加・その後訂正）: 「警告のみ」ポリシーで圏外のまま打刻が通った場合は
    「打刻」ではなく**「圏外打刻」カテゴリ（オレンジ色バッジ）**、`打刻拒否`ポリシーで打刻を拒否した場合は
    **「打刻拒否」カテゴリ（同じくオレンジ色バッジ）**で記録する（`src/app/admin/logs/page.tsx`の
    `actionClass()`）。**「エラー」カテゴリはシステム的な例外（DB書き込み失敗・締め処理失敗・税額表取込失敗など、
    管理者やシステムによる対応・復旧が必要な状況）専用**とし、圏外打刻・打刻拒否のような運用上想定内の状況には
    使わない方針にした（当初は打刻拒否も「エラー」で記録していたが訂正）。
