...
- **出勤（type=in）**: 当日の勤務レコードを作成/更新し `start_time` をセット。
  既に「未退勤（start あり・end なし）」のレコードがある場合は二重出勤とみなし、確認のうえ上書き or 無視。
- **退勤（type=out）**: **直近の「未退勤」レコード**（start あり・end なし。目安として直近18時間以内）に
...
- **休憩の自動入力**: 退勤時、`end_time - start_time`（日跨ぎ補正込み）の**総時間が6時間以上なら
  `break_minutes = 60`**、未満なら 0 を自動セット。**あとから勤務表で修正可能**。

### 7.3 位置情報（記録＋任意チェック）
- OK時にブラウザ **Geolocation API**（HTTPS必須＝充足）で緯度経度・精度を取得して記録する。
  許可されない場合は座標なしで打刻（＝**未許可でも打刻はできる**。記録は「位置なし」）。
- 設定画面（管理者）で以下を指定:
  - **基準住所**（ジオコーディングして基準座標を保持。ジオコーディングは保存時に1回実行。
    取得失敗時は緯度経度の直接入力にフォールバック）
  - **許容半径（m）**
...
- `work_entries` に打刻由来の補助列（任意）: `clock_in_at` / `clock_out_at`（打刻時刻の原本保持）等。
- `app_settings` に位置ポリシー: `clock_base_address` / `clock_base_lat` / `clock_base_lng` /
  `clock_radius_m` / `clock_out_of_range`(reject|warn) を追加。
...
### 7.6 未決事項（実装前に要確定）
- 二重出勤／未退勤のまま日付が変わった場合の扱い（自動締め切りの要否）。
- ジオコーディングの手段（利用サービス・住所→座標の精度）。当環境からの到達可否も確認が必要。
- QRの固定URLゆえ「その場にいなくても開ける」点の許容（現方針では位置チェックで抑止）。
