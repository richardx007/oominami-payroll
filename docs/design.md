...
- **時給・税区分の履歴編集UI（`admin/employees/ui.tsx`の`WageHistory`/`TaxHistory`。2026-07-24整理）**:
  ...
  編集フォームも「適用開始日（左）→値（右）」の順。フィールド行は`flex flex-col gap-2 sm:flex-row`
  （**スマホでは縦積み・sm以上でのみ横並び**）で`min-w-0`も付与し、`<input type=date>`が枠から
  はみ出す/隣の入力に重なる事故を防ぐ（横並びの`grid grid-cols-2`にしていた旧実装は、iOSの日付ウィジェットの
  実測幅が枠より広く隣接欄に重なる不具合が出たため撤去。2026-07-24）。一覧の下に新規追加フォーム
  （同じ左右順・同じ縦積みルール）があり、追加ボタンは右寄せ・「更新」ボタンと同サイズ（`px-4 py-2`）。
  税区分の表示行（適用年月日＋"甲欄/乙欄(扶養N人)"）は文字数が多く折り返しやすいため、
  ラベル部分に`whitespace-nowrap`、編集/削除ボタン部分に`ml-auto`を付けて、収まらない場合は
  ボタンごと次の行へ折り返す（文中の変な位置で改行されるのを防ぐ）。
