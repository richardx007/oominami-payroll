...
- カレンダー上部は、編集可能な画面(管理者)では「日をタップしてシフトを指定してください」、続けて
  「太字＝実績入力済み。赤太字＝予定と実績が相違」を表示（枠なしテキスト）。枠の時刻一覧（早番 8:00〜17:00 等）は
  **シフト編集パネルの上部**（管理者の編集画面のみ・枠背景なしの1行表示）に移動している。ホーム画面上部の
  期間ステータスバッジ（受付中/締め済み等）は2026-07-19にシフト予定表へ置換した際に廃止済み。
