...
- 各割当に**任意メモ**（`shift_assignments.note`。例「〜16」）を付与でき、ニックネームの後ろに小さく表示する。
  途中交代（分割シフト）は同じ枠に2人割り当て、各自のメモ（例「〜11」「11〜」）で表現する。
- **予実相違（`get_shift_status` が match 以外＝未実施 missing / 時刻ズレ timediff / 予定外 unplanned）の従業員名を太字の赤字**にする。
