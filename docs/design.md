...
### 13.3.2 ✕ 希望の時間帯の衝突表示（2026-08-04追加）

**調整中（draft）に、同じ日で異なる従業員の希望時間帯が重なっている日**は、カレンダーの
日付セル左上に**赤い「✕」**を出す。当人同士が気付いて自分で調整できるようにするのが目的。

- 判定は `collisionDates`（`ShiftSchedule.tsx`）。日ごとに全員の希望を
  `[開始, 終了)` の分数レンジへ直し、**別人同士**の総当たりで `r1.start < r2.end && r2.start < r1.end`
  なら重なりとみなす。
- **枠の既定時刻ではなく変則時刻を優先**する（`custom_start`/`custom_end` があればそちら）。
  同じ枠でも変則時刻を入れて時間をずらせば衝突しない、という運用に合わせるため。
- 🔴 **終了 ≤ 開始の枠（深夜 0:00〜9:00 など）は日をまたぐとみなし、終了に +24時間する。**
  そのまま比較すると深夜枠が「長さ0または負」になり、衝突を取りこぼす。業務日付ベースで
  すべて同じ日に寄せているのでこの処理で辻褄が合う（§業務日付を参照）。
- **ロックのみ（枠なし）の行は対象外**。重なる時間そのものが存在しないため。
