...
- **書式と操作の微調整（同日、オーナー確認後の依頼）**:
  - **スマホは「共有」だけを主ボタンに**（ダウンロードは出さない）、**PCは「ダウンロード」だけ**。
    判定は `navigator.canShare({files})`。UA判定ではなく機能判定にしている。
  - 明細書: **年月度を大きく・太字**、**左右の余白を24mm に拡大**（A4幅いっぱいだと項目名と金額が
    離れすぎて読みにくい）、**支払元のかたまりを1行ぶん下げ**、**支払元1行目を大きめ・太字**。
  - 給与明細画面の表: **交通費より右の列（rowSpan でまとめている列）から `align-top` を外した**。
    PDFボタンで行が少し高くなり、左隣の列（既定＝中央）と縦位置がずれて見えていたため。
    ⚠️ 氏名列は「未確定」の注記が2行になることがあるので `align-top` のまま。
