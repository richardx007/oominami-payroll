...
- **iPhoneで時刻の「クリア」が効かない不具合**: 原因は入力欄の
  `value={cs || (toInputTime(slots[cur].start) ?? "")}` というフォールバック。
  **入力欄が常に値を持つ**ため、クリアしても枠の既定時刻が即座に再表示され空にできなかった
  （＝一度時刻を入れると「変則でない」状態に戻せない）。`value={cs}` にして真に空を許容し、
  枠の既定時刻は見出しの脇に「(既定 22:00〜7:00)」と参考表示する形に変えた。
  あわせて**「クリア」ボタン**を追加（入力済みのときだけ表示。出勤・退勤をまとめて空に戻す）。
  - ボタンは `onMouseDown` の既定動作を止めている。クリックで入力欄が blur すると
    `onBlur` の保存が先に走って**二重保存**になるため。
- ⚠️ **今後の教訓**: `<input type="time">` に「空のときは既定値を表示」を `value` で実装しないこと。
  プレースホルダーのつもりでも実体は値なので、ユーザーが空に戻せなくなる。
  参考表示したいなら入力欄の外に出す。
