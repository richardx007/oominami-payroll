...
- **【再発2回目】`useTransition`の pending 固着**: pending はサーバーアクションの完了ではなく
...
- **シフトの「確定/調整中」モード(2026-07-27追加)**: 月ごとに `shift_modes` で保持。調整中は従業員が
  **自分の希望だけ**を入力でき**他人の希望は見えない**、確定は従来どおり(管理者のみ編集・全員閲覧可)。
  希望と確定は**同じ `shift_assignments`** に持ち、モード切替でデータは移動せず RLS の判定だけが変わる。
  ...
  **他人の希望を隠す防御は2箇所必要**: `shift_assignments` のSELECTポリシーと、
  **`get_shift_status()` の where 句**。後者を忘れると、割当が隠れていてもこの SECURITY DEFINER 関数経由で
  「誰がどの日に希望を出しているか」が漏れる。画面側の名簿の絞り込み(`(employee)/shifts/page.tsx`)は
  UXのためで防御の主体ではない。`assignShift`/`clearShift` は管理者専用から「管理者 or 調整中の本人」に
  緩和済み(`canEditShift()`。最終的な担保はRLS)。詳細は設計書§13。
