...
- **対応: API互換のフォーク `html2canvas-pro`(2.3.3) に移行**し、oklch に対応させた。
  給与明細・QRコードの**両方**を移行し、本家 `html2canvas` は依存から削除した(**再導入しないこと**)。
  `npm audit` の件数は導入前後で変化なし(5件。いずれも既存の wrangler/sharp 由来)。
- **エラーを握りつぶさないようにした**: `catch` で汎用文言だけを出していたため原因が全く追えなかった。
  実際のエラーメッセージを画面に併記するよう変更(`PDFの作成に失敗しました(＜詳細＞)`)。
  今後この種の機能を足すときも、**まず詳細を出す**こと。
- ⚠️ **要再確認**: QRコードのPDFもライブラリを差し替えたため、**動作していたものが壊れていないか
  実機での再確認が必要**。
