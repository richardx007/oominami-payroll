...
- **⚠️ `supabase/migrations/` は完全な履歴ではない**: DBには32件適用済みだが、初期にダッシュボードから
  ...
  **`supabase/schema.sql`**(2026-07-28時点のスナップショット。バックアップの schema.sql の写し)か、
...
- **適用した制約**（`supabase/migrations/20260801_advance_payments_fk.sql`。本番適用済み）:
  ```sql
...
- **未入力の欄を開いたときの初期値を `00:00` にした**（`onFocus` で投入）。
  ...
  → **オーナーに報告済み。修正するかは要判断**（業務データのため勝手に変更していない）。
