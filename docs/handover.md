...
- [ ] **シフトのカレンダー連携（ICS購読フィード）**: 従業員要望。**Yahoo!カレンダーへの自動連携は不可**
      （インポート機能なし・CalDAVは2021年終了・API非公開）と結論。代わりに従業員ごとの ICS 購読
      フィード（iPhone標準カレンダー/Googleカレンダーで購読）を推奨。着手前にオーナー判断が要る点が
      3つある。詳細は下記「Yahoo!カレンダー連携の検討（2026-09-16）」参照。
